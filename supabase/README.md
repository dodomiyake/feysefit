...
8. **Customer (marketplace):** Customer signs up as direct → browses `/marketplace` → sends an enquiry. Run **`patch-marketplace-enquiries.sql`** after the security follow-up patches. The designer accepts or declines; acceptance activates only that designer–customer pair, and the designer then creates the project from `/enquiries`. Do not use `patch-marketplace-request-rls.sql` for a current deployment.
...
29. **Historical marketplace link RPC:** `patch-marketplace-link-rpc.sql` is superseded by `patch-marketplace-enquiries.sql`. The current flow never switches off a client’s other legitimate designer relationships; each accepted enquiry and project remains tied to its selected designer.
