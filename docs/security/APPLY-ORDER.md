...
11. `supabase/patch-concurrent-session-detection.sql` (adds `list_own_active_sessions()` — a self-service, `auth.uid()`-scoped read of `auth.sessions` via the existing `redact_ip_hint()`/`coarse_device_hint()` projections, exposed as the account security page's "Active sessions" list — and adds `concurrent_session_detected` to `log_account_activity_server`'s allowed event types so a login can be flagged when another session is already active. Ship the matching application in the same release: `POST /auth/session/start` calls both after every login. Depends on `patch-account-security.sql` and `patch-security-audit-followup-2.sql` for `log_account_activity_server`, `redact_ip_hint`, and `coarse_device_hint`.)
