...
export function DesignerMarketplaceCard({ designer }: { designer: Designer }) {
  ...
  return (
    <article className="overflow-hidden rounded-2xl bg-card shadow-sm transition-all hover:shadow-md">
      <Link href={profileHref} className="relative block aspect-[4/3] w-full lg:aspect-[5/4]">
        <Image
      ...
      <div className="space-y-4 p-5 lg:p-6">
        <div>
          <Link href={profileHref} className="group inline-flex items-center gap-1.5">
            <h3 className="font-headline text-lg font-semibold text-primary group-hover:text-accent lg:text-xl">
        ...
        <div className="hidden flex-wrap gap-2 lg:flex">
          {meta.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-md border border-primary/10 bg-background/60 px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider text-primary/65"
            >
              {tag}
            </span>
          ))}
